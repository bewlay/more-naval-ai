...
def getCustomMapOptionDescAt(argsList):
	...
	selection_names = ["TXT_KEY_MAP_SCRIPT_SNAKY_CONTINENTS",
	                   "TXT_KEY_MAP_SCRIPT_ARCHIPELAGO",
		...
		for x in range(iWestX, iEastX + 1):
			for y in range(iSouthY, iNorthY + 1):
				...
				if pPlot.isWater(): # Water plot
					iFertileCheck = pPlot.calculateBestNatureYield(YieldTypes.YIELD_FOOD, PlayerTypes.NO_PLAYER)
					...
					iNumLandPlots += 1
					iArea = pPlot.getArea()
					iPlotYield = pPlot.calculateTotalBestNatureYield(PlayerTypes.NO_PLAYER)
					iFertileCheck = pPlot.calculateBestNatureYield(YieldTypes.YIELD_FOOD, PlayerTypes.NO_PLAYER)
