...
#
#   2.06f.1_ms  Temudjin  16.Mar.2011
#   - included Fuyu's balance changes (mostly) - Thanks Fuyu, LunarMongoose, OnmyojiOmn
#   - fixed, compatibility with Mars Now!
...
#
#	2.06_mst   Temudjin  15.Jul.2010  ( uses MapScriptTools )
...
def getDescription():
	return "TXT_KEY_MAP_SCRIPT_PERFECT_WORLD_DESCR"
...
def beforeInit():
	...
	global mapOptionNewWorld, mapOptionClimate, mapOptionPangea, mapOptionWorldShape, mapOptionResources
	...
	mapOptionNewWorld = map.getCustomMapOption(2)
	mapOptionPangea = map.getCustomMapOption(3)
...
def beforeGeneration():
	...
	balancer.initialize( mapOptionResources == 1, mst.iif(mst.bFFH, True, False), True )	# balance boni if desired, add missed boni for FFH only, move minerals
...
def addBonuses():
	print "--- addBonuses()"
	if mst.bMars:
		CyPythonMgr().allowDefaultImpl()							# generate default boni
	else:
		bp.AddBonuses()												# use Perfect World bonus system

# ######################################################################################################
# ######## assignStartingPlots() - Called from system
# ######## - assign starting positions for each player after the map is generated
# ######## - Planetfall and Mars - use default implementation
# ######################################################################################################
...
def normalizeStartingPlotLocations():
	...
	if mst.bMars:
		...
		CyPythonMgr().allowDefaultImpl()
	elif mapOptionTeamStart == 0:							# by default civ places teams near to each other
		CyPythonMgr().allowDefaultImpl()
	elif mapOptionTeamStart == 1:							# shuffle starting-plots to separate teams
...
def setCustomOptions():
	...
	lMapOptions = [1, 0, 0, 0, 0, 0]

	# Try to read map options from the cfgFile.
	mst.mapOptionsStorage.initialize(lMapOptions, 'PerfectWorld2')
	lMapOptions = mst.mapOptionsStorage.readConfig()

	op = {
	       0: ["TXT_KEY_MAP_SCRIPT_WORLD_WRAP", ["TXT_KEY_MAP_SCRIPT_WORLD_WRAP_FLAT", "TXT_KEY_MAP_SCRIPT_WORLD_WRAP_CYLINDRICAL", "TXT_KEY_MAP_SCRIPT_WORLD_WRAP_TOROIDAL"], lMapOptions[0], True],
		   1: ["TXT_KEY_MAP_RESOURCES", ["TXT_KEY_MAP_RESOURCES_STANDARD", "TXT_KEY_MAP_RESOURCES_BALANCED"], lMapOptions[1], True],
	       2: ["TXT_KEY_MAP_PERFECTWORLD_NEW_WORLD_RULES", ["TXT_KEY_MAP_PERFECTWORLD_NEW_WORLD_ANYWHERE", "TXT_KEY_MAP_PERFECTWORLD_NEW_WORLD_OLD_WORLD"], lMapOptions[2], True],
	       3: ["TXT_KEY_MAP_PERFECTWORLD_PANGAEA_RULES",   ["TXT_KEY_MAP_PERFECTWORLD_PANGAEA_BREAK", "TXT_KEY_MAP_PERFECTWORLD_PANGAEA_ALLOW"], lMapOptions[3], True],
	...
	if mst.bMars or mst.bPfall:
		op[2] = ["TXT_KEY_MAP_PERFECTWORLD_NEW_WORLD_RULES", ["TXT_KEY_MAP_PERFECTWORLD_NEW_WORLD_ANYWHERE"], 0, False]
	#mst.printDict(op,"PerfectWorld Map Options:")
...
def getNumHiddenCustomMapOptions():
	...
	mc.initialize()							# Initialize PerfectWorld Constants
...
def getGridSize(argsList):
	...
	# The original grid sizes by Cephalo can be roughly approximated as being one size larger than the one selected.
	[eWorldSize] = argsList

	if eWorldSize < (WorldSizeTypes.NUM_WORLDSIZE_TYPES - 1):
		...
		# Scale up the biggest world size
		iScale = 1.2
		iWidth = int(math.ceil(CyGlobalContext().getWorldInfo(WorldSizeTypes.NUM_WORLDSIZE_TYPES - 1).getGridWidth() * iScale))
		iHeight = int(math.ceil(CyGlobalContext().getWorldInfo(WorldSizeTypes.NUM_WORLDSIZE_TYPES - 1).getGridHeight() * iScale))
...
## GLOBAL TUNING VARIABLES: Change these to customize the map results
		...
		# zero and (number of players)/2. When set to 1, this will disable all bonus grouping.
		self.BonusMaxGroupSize = 4
		...
		self.allowWonderBonusChance = 0.0667
	...
	def initInGameOptions(self):
		#New World Rules
		self.AllowNewWorld = (mapOptionNewWorld == 1)
		if mst.bMars or mst.bPfall:
			self.AllowNewWorld = False

		#Pangaea Rules
		self.AllowPangeas = (mapOptionPangea == 1)

		#Wrap options
		wrapString = "Cylindrical"
		if mapOptionWorldShape == 2: #Toroidal
			self.hmHeight -= 1
			self.WrapY = True
		...
		elif mapOptionWorldShape == 0: #Flat
			self.hmWidth += 1
			self.WrapX = False
			wrapString = "Flat"
...
def ShuffleList(theList):
	...
	return shuffled
	"""
	return mst.randomList.xshuffle( theList )

def GetInfoType(string):
	return gc.getInfoTypeForString(string)
    ...
    def AddBonusType(self,eBonus,plotIndexList):
        ...
        for i in range(len(plotIndexList)):
            ...
            if self.CanPlaceBonusAt(plot,eBonus,False,False):
                ...
                ##### Fuyu START - some changes by Temudjin
                ...
                if (mc.BonusMaxGroupSize == -1):
                    ...
                    maxAdd = PRand.randint(1, gc.getGame().countCivPlayersEverAlive()/2);
                else:
...
#        game.gc.getGame()
    ...
    def getPlotPotentialValue(self,x,y,coastalCity):
        ...
        commerce += mst.calculateBestNatureYield(plot, YieldTypes.YIELD_COMMERCE)
        food += mst.calculateBestNatureYield(plot, YieldTypes.YIELD_FOOD)
        production += mst.calculateBestNatureYield(plot, YieldTypes.YIELD_PRODUCTION)
        ...
        for n in range(gc.getNumBuildInfos()):
            ...
            if mst.canHaveImprovement(plot, impEnum, True) == True:
...
def makeChannel(x,y):
    ...
    plot = mmap.plot(x,y)
    cleanUpLake(x,y)
    plot.setTerrainType(mst.etCoast,True,True)
    plot.setRiverID(-1)
    plot.setNOfRiver(False,CardinalDirectionTypes.NO_CARDINALDIRECTION)
    plot.setWOfRiver(False,CardinalDirectionTypes.NO_CARDINALDIRECTION)
    replaceRivers(x,y)
...
def addFeatures2():
    ...
    mmap = gc.getMap()
    featureJungle = mst.efJungle
    featureForest = mst.efForest
    featureOasis = mst.efOasis
    featureFloodPlains = mst.efFloodPlains
    FORESTLEAFY = 0
    FORESTEVERGREEN = 1
    FORESTSNOWY = 2
