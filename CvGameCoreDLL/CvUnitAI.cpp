...
/*************************************************************************************************/
/**	BETTER AI (faster barbs ) Sephi 0.41k                                      					**/
/**	                        																	**/
...
/*************************************************************************************************/
    ...
    {
        if (GC.getGameINLINE().getSorenRandNum(400, "break free") == 0)
        {
            setOriginPlot(NULL);
        }
	}
		...
		for (iDX = -(iRange); iDX <= iRange; iDX++)
		{
			for (iDY = -(iRange); iDY <= iRange; iDY++)
			{
				...
				if (pLoopPlot && pLoopPlot!=plot())
				{
					...
					{
						//look for things to smash
						if (!bTimeToPillage)
						{
							if(pLoopPlot->isVisibleEnemyUnit(this) && GET_PLAYER(getOwnerINLINE()).AI_getEnemyPlotStrength(pLoopPlot,0,true,false)>0)
							{
								if(generatePath(pLoopPlot,0,false,&iPathTurns))
								{
                                    ...
                                    //barbs like to plunder civilization
...
bool CvUnitAI::isUnitAllowedExploreGroup()
{
                                ...
                                if (GET_TEAM(getTeam()).isAtWar(pLoopPlot->getTeam()))
                                {
                                        ...
                                        baseCombatStr() > pBestDefender->baseCombatStrDefense())
                                    {
                                        if (generatePath(pLoopPlot, 0, true, &iPathTurns))
                                        {
											int iOurStrength=getGroup()->AI_GroupPower(pLoopPlot,false);
											int iTheirStrength=GET_PLAYER(getOwnerINLINE()).AI_getEnemyPlotStrength(pLoopPlot,0,true,false);
											if (getGroup()->getNumUnits()>=pLoopPlot->getNumVisibleEnemyDefenders(this)*3)
											{
											...
											if (iOurStrength>(iTheirStrength*1.2)  && (iTheirStrength>0))
											{
												// Tholal AI - added *2 to make them more aggressive
												iValue=((iOurStrength*100/iTheirStrength)*2)/(1+iPathTurns);
												if (iValue>iBestValue)
												{
													iBestValue=iValue;
													pBestPlot=pLoopPlot;
...
{
	if (!(getUnitType() == (UnitTypes)GC.getDefineINT("UNIT_VAMPIRE")
          || getUnitType() == (UnitTypes)GC.getDefineINT("UNIT_VAMPIRE_LORD")))
	{
		AI_setUnitAIType(UNITAI_ATTACK);
		getGroup()->pushMission(MISSION_SKIP);
		return;
	}
    else if (getLevel()>5 || !(plot()->isCity()))
	{
	...
	if (pCity)
	{
		if (pCity->getPopulation()>13 || pCity->angryPopulation()>1 || pCity->foodDifference() < 0 || pCity->unhealthyPopulation(false) > 3)
		{
...
void CvUnitAI::PatrolNewMove()
{
    ...
    int flag;
    CvSelectionGroup* pBestGroup=NULL;
    ...
    // Search for A City nearby to join its Patrol
    for (iDX = -(iSearchRange); iDX <= iSearchRange; iDX++)
    {
        for (iDY = -(iSearchRange); iDY <= iSearchRange; iDY++)
        {
            ...
            if (pLoopPlot != NULL)
            {
                if(pLoopPlot->isCity())
                {
                    if(pLoopPlot->getOwnerINLINE()==getOwnerINLINE() && (generatePath(pLoopPlot, 0, true, &iPathTurns)))
                    {
                        //found a City, What's its status?
                        CvCity* targetCity=pLoopPlot->getPlotCity();

                        if (!targetCity->AI_getCityPatrolOrigin())
                        {
                            //is a City closeby with a Patrolgroup?
                            int iSearchRange2=6;
                            int iDX2,iDY2;
                            CvPlot* pLoopPlot2;
                            for (iDX2 = -(iSearchRange2); iDX2 <= iSearchRange2; iDX2++)
                            {
                                for (iDY2 = -(iSearchRange2); iDY2 <= iSearchRange2; iDY2++)
                                {
                                    pLoopPlot2 = plotXY(targetCity->getX_INLINE(), targetCity->getY_INLINE(), iDX2, iDY2);

                                    if (pLoopPlot2 != NULL)
                                    {
                                        if(pLoopPlot2->isCity())
                                        {
                                            if(pLoopPlot2->getOwnerINLINE()==getOwnerINLINE() && pLoopPlot2->getPlotCity()->AI_getCityPatrolOrigin())
                                            {
                                                if (pLoopPlot2==pLoopPlot2->getPlotCity()->AI_getCityPatrolOrigin())
                                                {
                                                    if (generatePath(pLoopPlot2, 0, true, &iPathTurns))
                                                    {
                                                        targetCity->AI_setCityPatrolOrigin(pLoopPlot2->getPlotCity()->AI_getCityPatrolOrigin());
                                                    }
                                                }
                                            }
                                        }
                            ...
                            //Else make the City the center of a new Patrolgroup
                            if (!targetCity->AI_getCityPatrolOrigin())
                            {
                                targetCity->AI_setCityPatrolOrigin(targetCity->plot());
                            }
                        }

                        //What's the Cities Patrolgroup?
                        CvSelectionGroup* Patrolgroup=NULL;
                        int iLoop;
                        ...
                        for (CvUnit* pUnit = GET_PLAYER(getOwnerINLINE()).firstUnit(&iLoop); NULL != pUnit; pUnit = GET_PLAYER(getOwnerINLINE()).nextUnit(&iLoop))
                        {
                            if (pUnit->AI_getGroupflag()==GROUPFLAG_PATROL)
                            {
                                if (pUnit->getOriginPlot()==targetCity->AI_getCityPatrolOrigin())
                                {
                                    Patrolgroup=pUnit->getGroup();
                                    break;
                                }
                        ...
                        if (generatePath(targetCity->AI_getCityPatrolOrigin(), 0, true, &Value))
                        {
                            if(Value<BestValue)
                            {
                                if (!Patrolgroup)
                                {
									BestValue=Value;
                                    newGroup=true;
                                    pBestCity=targetCity;
                                }
                                else if (targetCity->AI_neededPatrol(flag)>0)
                                {
									BestValue=Value;
                                    newGroup=false;
                                    pBestGroup=Patrolgroup;
                                }
                            }
                        }
                    }
    ...
    if (newGroup && pBestCity!=NULL)
    {
        ...
        setOriginPlot(pBestCity->AI_getCityPatrolOrigin());
        getGroup()->pushMission(MISSION_SKIP);
        return;
    }
    else if (!newGroup && pBestGroup!=NULL)
    {
        if (atPlot(pBestGroup->plot()))
        {
            AI_setGroupflag(GROUPFLAG_PATROL);
            if (flag==0)
            {
            ...
            setOriginPlot(pBestGroup->getHeadUnit()->getOriginPlot());

			if (pBestGroup->getHeadUnit()->getOriginPlot())
			{
				if (pBestGroup->getHeadUnit()->getOriginPlot()->getPlotCity())
				{
					pBestGroup->getHeadUnit()->getOriginPlot()->getPlotCity()->AI_calculateNeededPatrol(); //recalculate Patrol
				}
			}

            joinGroup(pBestGroup);
        }
        else
        {
			getGroup()->pushMission(MISSION_MOVE_TO, pBestGroup->getHeadUnit()->getX_INLINE(), pBestGroup->getHeadUnit()->getY_INLINE(), MOVE_DIRECT_ATTACK);
        }
        return;
    }
...
void CvUnitAI::PatrolMove()
{
	...
	//every patrol should have a Citygroup it wants to defend
    if(getOriginPlot()==NULL)
    {
        getGroup()->pushMission(MISSION_SKIP);
        joinGroup(NULL);
	...
	//Do we need to join Patrol group again?
    if (getGroup()->getNumUnits()==1)
	{
		if (RejoinPatrolGroup())
		{
			return;
		}
	}

    //is there a City under Siege?
    if (AI_patrol_guardcity())
    {
        return;
    }
        ...
        if (pLoopCity->AI_getCityPatrolOrigin())
        {
            if (pLoopCity->AI_getCityPatrolOrigin()==getOriginPlot())
            {
                ...
                while (pUnitNode != NULL)
                {
                    ...
                    if (pLoopUnit)
                    {
                        // There is a Settler! Either move to where he is or regroup
                        if (pLoopUnit->AI_getUnitAIType()==UNITAI_SETTLE && AI_canJoinGroup(pLoopUnit->getGroup()))
                        {
                            if (pLoopUnit->getGroup()->getNumUnits()<4)
                            {
                                if (atPlot(pLoopUnit->plot()))
                                {
									...
									while (iEnough<4)
									{
										//search for a unit without much experience and add it to the Settlergroup
										CvUnit* pBestUnit=NULL;
										int iBestValue=100;
										CLLNode<IDInfo>* pUnitNode1;
										CvUnit* pLoopUnit1;
										pUnitNode1 = plot()->headUnitNode();
										while (pUnitNode1 != NULL)
										{
											pLoopUnit1 = ::getUnit(pUnitNode1->m_data);
											pUnitNode1 = plot()->nextUnitNode(pUnitNode1);
											if (pLoopUnit1!=NULL)
											{
												if(pLoopUnit1->AI_getGroupflag()==GROUPFLAG_PATROL && pLoopUnit1->getOriginPlot()==getOriginPlot())
												{
													if(pLoopUnit1!=this || getGroup()->getNumUnits()==1)
													{
														if(pLoopUnit->getGroup()->getNumUnits()<4)
														{
															if(pLoopUnit1->getExperience()<iBestValue)
															{
																pBestUnit=pLoopUnit1;
																iBestValue=pLoopUnit1->getExperience();
															}
														}
													}
												}
										...
										if (pBestUnit!=NULL)
										{
											...
											pBestUnit->joinGroup(pLoopUnit->getGroup());
											pBestUnit->AI_setGroupflag(GROUPFLAG_PERMDEFENSE_NEW);
											pBestUnit->setOriginPlot(NULL);
											pBestUnit->getGroup()->pushMission(MISSION_SKIP);
											iEnough++;
										}
										//didn't find a unit to add to the settler, abort the loop
										else
										{
											iEnough=1000;
										}
	        ...
	        if (plot()->getPlotCity()->AI_neededPermDefense(0)>0)
	        {
                ...
                while (pUnitNode != NULL)
                {
                    ...
                    if (pLoopUnit)
                    {
                        if (pLoopUnit->getOwnerINLINE()==getOwnerINLINE())
                        {
                            if (pLoopUnit->AI_getGroupflag()==GROUPFLAG_PATROL)
                            {
                                if (flag==0)
                                {
                                    pLoopUnit->AI_setGroupflag(GROUPFLAG_PERMDEFENSE_NEW);
...
//first hit non barb units
    for (pLoopCity = GET_PLAYER(getOwnerINLINE()).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(getOwnerINLINE()).nextCity(&iLoop))
    {
        if (pLoopCity->AI_getCityPatrolOrigin())
        {
            if (pLoopCity->AI_getCityPatrolOrigin()==getOriginPlot())
            {
                CvPlot* searchPlot=pLoopCity->plot();
                ...
                for (iDX = -(iSearchRange); iDX <= iSearchRange; iDX++)
                {
                    for (iDY = -(iSearchRange); iDY <= iSearchRange; iDY++)
                    {
                        pLoopPlot = plotXY(searchPlot->getX_INLINE(), searchPlot->getY_INLINE(), iDX, iDY);
                        ...
                        if (pLoopPlot != NULL)
                        {
                            if ((AI_plotValid(pLoopPlot)) && pLoopPlot->getOwnerINLINE()==getOwnerINLINE())
                            {
                                if (pLoopPlot->isVisibleEnemyUnit(this))
                                {
									CvUnit* pBestDefender=pLoopPlot->getBestDefender(NO_PLAYER,NO_PLAYER,this);
									if (!pBestDefender->isBarbarian())
									{
										if (!atPlot(pLoopPlot) && ((bFollow) ? canMoveInto(pLoopPlot, true) : (generatePath(pLoopPlot, 0, true, &iPathTurns) )))
										{
											if (pLoopPlot->getNumVisibleEnemyDefenders(this) >= iMinStack)
											{
												int iOurStrength=getGroup()->AI_GroupPower(pLoopPlot,false);
...
void CvUnitAI::SettlergroupMove()
{
    if (!plot()->isCity() || (plot()->getOwnerINLINE()!=getOwnerINLINE()))
    {
  	...
  	//Are we in A City that needs Defense?
    else if (plot()->getPlotCity()->AI_neededPermDefense(0)>0)
    {
        //Split so we can join the Defense
...
void CvUnitAI::ConquestMove()
{
	...
	/*** Better War AI: Moved to increase priority for defending captured cities ***/
	//Are we in A City that needs Defense?
	if (plot()->isCity() && plot()->getOwnerINLINE()==getOwnerINLINE()) //&& getGroup()->getNumUnits()>30)
	{
		int iNeededDefenders = std::min(2, plot()->getPlotCity()->AI_neededPermDefense(0));

        if (iNeededDefenders > 0)
        {
			//Split so we can join the Defense
			bool bHasJoinedCity = false;
            ...
            for (int iI = 0; iI < iNeededDefenders; iI++)
            {
                ...
                while (pUnitNode != NULL)
                {
                    ...
                    if (pLoopUnit!=NULL)
					{
						iValue=pLoopUnit->getExperience();
                ...
                {
                    pBestUnit->joinGroup(NULL);
					pBestUnit->AI_setUnitAIType(UNITAI_CITY_DEFENSE);
					...
					// Need to do this or *all* our units will become perm-defenders
					// if in separate selection groups
					plot()->getPlotCity()->AI_calculateNeededPermDefense();
                }
                else
                {
					//didn't find a unit , abort the loop
                    break;
                }
            }
    ...
    if (plot()->getOwnerINLINE()==getOwnerINLINE())
    {
        ...
        else
        {
			...
			*/
			// Tholal Merge - another Snarko fix - 6/15/10
			bool splitup=false;
            for(pLoopSelectionGroup = GET_PLAYER(getOwnerINLINE()).firstSelectionGroup(&iLoop); pLoopSelectionGroup != NULL; pLoopSelectionGroup = GET_PLAYER(getOwnerINLINE()).nextSelectionGroup(&iLoop))
            {
                if (pLoopSelectionGroup->getHeadUnit() != NULL)
	...
	if (!bCity)
	{
		...
		int iMinAttackSize=std::max(1,getGroup()->getNumUnits()/3);

        if (AI_anyAttack(2, 65, iMinAttackSize))
        {
            return;
        }
		...
		if (getGroup()->getNumUnits() > 1)
		{
			if (AI_stackAttackCity(1, 110, false))			//Need to Tweak this Value
			{
				return;
			}
		}
