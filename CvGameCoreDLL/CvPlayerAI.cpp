...
//						iUnitValue += ((getStateReligion() != NO_RELIGION) ? 600 : 300);
						iUnitValue += ((getStateReligion() == NO_RELIGION || 
							getStateReligion() == GC.getUnitInfo(eLoopUnit).getReligionType() || 
							getFavoriteReligion() == GC.getUnitInfo(eLoopUnit).getReligionType()) ? 400 : 0);
//<<<<Better AI: End Modify
						break;
...
//<<<<Advanced Rules: End Add
					...
					*/
					...
					{
						if (GC.getUnitInfo(eLoopUnit).getPrereqOrBonuses(iI) != NO_BONUS)
						{
							bRequiresBonus = true;

							if (hasBonus((BonusTypes)GC.getUnitInfo(eLoopUnit).getPrereqOrBonuses(iI)))
							{
								bHasRequiredBonus = true;
							}
						}
					...
					if (GC.getUnitInfo(eLoopUnit).getPrereqAndBonus() != NO_BONUS)
					{
						...
						if (hasBonus((BonusTypes)GC.getUnitInfo(eLoopUnit).getPrereqAndBonus()))
						{
							bHasRequiredBonus = true;
						}
					}
				...
				for (iII = 0; iII < GC.getNumUnitClassInfos(); iII++)
				{
					...
					if (eLoopUnit != NO_UNIT)
					{
						if (GC.getUnitInfo(eLoopUnit).getDomainType() == DOMAIN_LAND)
						{
							...
							else if (GC.getUnitInfo(eUnit).getUnitCombatType() != NO_UNITCOMBAT)
							{
								if (GC.getUnitInfo(eLoopUnit).getUnitCombatCollateralImmune((UnitCombatTypes)GC.getUnitInfo(eUnit).getUnitCombatType()))
								{
...
/************************************************************************************************/
/* Afforess	                  Start		 06/01/10                                               */
/* Ruthless AI: AI will purchase war allies                                                     */
/************************************************************************************************/
									//if (GC.getGameINLINE().isOption(GAMEOPTION_AGGRESSIVE_AI))
									if (1 < 2)
									{
										...
										if (GET_PLAYER((PlayerTypes)iI).isHuman() || ((GET_TEAM(getTeam()).getLeaderID() == getID()) && !GET_TEAM(getTeam()).isVassal(GET_PLAYER((PlayerTypes)iI).getTeam())))
										{
											if ((AI_getMemoryCount(((PlayerTypes)iI), MEMORY_DECLARED_WAR) == 0) && (AI_getMemoryCount(((PlayerTypes)iI), MEMORY_HIRED_WAR_ALLY) == 0))
											{
												if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_JOIN_WAR) == 0)
												{
													if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_JOIN_WAR), "AI Diplo Trade War") == 0)
													{
										...
										if (GET_PLAYER((PlayerTypes)iI).isHuman() || ((GET_TEAM(getTeam()).getLeaderID() == getID()) && !GET_TEAM(getTeam()).isVassal(GET_PLAYER((PlayerTypes)iI).getTeam())))
										{
											if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_STOP_TRADING) == 0)
											{
												if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_STOP_TRADING), "AI Diplo Trade War") == 0)
												{
