...
int CyPlayer::AI_unitValue(int /*UnitTypes*/ eUnit, int /*UnitAITypes*/ eUnitAI, CyArea* pArea, bool bUpgrade)
{
	return m_pPlayer ? m_pPlayer->AI_unitValue((UnitTypes)eUnit, (UnitAITypes)eUnitAI, pArea->getArea(), bUpgrade) : -1;
}
...
int CyPlayer::getOpenPlayer() const
{
	return m_pPlayer ? (int)m_pPlayer->getOpenPlayer() : (int)NO_PLAYER;
}

int /*PlayerTypes*/ CyPlayer::initNewEmpire(int /*LeaderHeadTypes*/ eNewLeader, int /*CivilizationTypes*/ eNewCiv)
{
	if (m_pPlayer)
	{
		return m_pPlayer->initNewEmpire((LeaderHeadTypes)eNewLeader, (CivilizationTypes)eNewCiv);
	}

	return NO_PLAYER;
}

void CyPlayer::setParent(int /*PlayerTypes*/ eParent)
{
	if (m_pPlayer)
	{
		m_pPlayer->setParent((PlayerTypes)eParent);
	}
}
...
int CyPlayer::getArcaneTowerVictoryFlag() const
{
    return m_pPlayer ? m_pPlayer->getArcaneTowerVictoryFlag() : -1;
}
