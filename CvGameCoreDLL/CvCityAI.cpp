...
// Tholal ToDo - better formulas - or better yet, incorporate this into the choose unit function instead
	...
	//if (kPlayer.hasTrait(GC.getInfoTypeForString("TRAIT_SPIRITUAL")))
		iNeededPriests *= 2;
...
// End Tholal AI
	...
	if (isBarbarian())
	{
		...
		// Tholal ToDO: Seen this result in lots of barbarian workboats doing nothing - maybe issue with neededseaworkers check?
...
// Tholal AI - Build Victory Buildings
    ...
    if (bDanger) 
    {
		int iAttackNeeded = 6; // Tholal AI - changed this to 6 (from 4)
	...
	{
		if (AI_chooseBuilding(BUILDINGFOCUS_GOLD, 8, 10 + 2*iWarTroubleThreshold, 50))
		{
			return;
		}
	}
...
//FfH: End Add
// Tholal AI - Klutzy add to catch civ traits - ToDo: figure out why AI above doesnt value these units highly
...
// End Tholal AI
								// Tholal AI - Extra value for heroes
...
// - Tholal AI - various FFH tweaks
// Tholal ToDo: Figure out why some buildings are getting too low priority
		...
		if  (kBuilding.getBuildingClassType() == (GC.getInfoTypeForString("BUILDINGCLASS_TRAINING_YARD") || GC.getInfoTypeForString("BUILDINGCLASS_SIEGE_WORKSHOP")))
		{
			if (kOwner.getNumBuilding(eBuilding) < (kOwner.getNumCities() / 3))
			{
				iValue++;
...
/************************************************************************************************/
			...
			{
				// trade routes
				// Tholal ToDo: Change this? (4 below used to be an 8)
				iTempValue = ((kBuilding.getTradeRoutes() * ((4 * std::max(0, (totalTradeModifier() + 100))) / 100))
								* (getPopulation() / 5 + 1));
...
// Tholal AI - changed based defenders to 2; added check for Barbarian Allies
...
int CvCityAI::AI_getTargetSize()
{
	...
	if (kPlayer.getAdvancedStartPoints() >= 0)
	{
		...
		//iTargetSize += 2 + getCurrentPeriod();
	}
...
/************************************************************************************************/
	    if (GC.getGameINLINE().AI_combatValue(getConscriptUnit()) > 33)
        {
            ...
            if (bLandWar && (0 == angryPopulation(iHappyDiff)))
            {
				...
				if( bWait && GET_PLAYER(getOwnerINLINE()).AI_isDoStrategy(AI_STRATEGY_LAST_STAND) )
				{
					// Full out defensive
...
// runs once every turn in CvPlayer::doTurn
void CvCityAI::AI_calculateNeededPatrol()
{
    for (int iFlag=0;iFlag<NUM_PATROL_TYPES;iFlag++)
    {
        ...
        switch(iFlag)
        {
            case 0: //normal units
                patrolneeded=3;
                patrolneeded*=(GC.getLeaderHeadInfo(GET_PLAYER(getOwnerINLINE()).getLeaderType()).getAIPatrolGroupSize()/100);
                if (isCapital())
                {
                    patrolneeded+=1;
                    patrolneeded=std::max(4,patrolneeded);
                }
            ...
            case 1:
                patrolneeded=1;
                patrolneeded*=(GC.getLeaderHeadInfo(GET_PLAYER(getOwnerINLINE()).getLeaderType()).getAIPatrolGroupMage()/100);
                break;
        }

        int countpatrolunits=0;
        int iLoop;
        if (AI_getCityPatrolOrigin())
        {
            for(CvUnit* pLoopUnit = GET_PLAYER(getOwnerINLINE()).firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = GET_PLAYER(getOwnerINLINE()).nextUnit(&iLoop))
            {
                if (pLoopUnit)
                {
                    if(pLoopUnit->getOriginPlot())
                    {
                        if(pLoopUnit->getOriginPlot()==AI_getCityPatrolOrigin() && (pLoopUnit->AI_getGroupflag()==GROUPFLAG_PATROL))
                        {
                            if ((iFlag==0) && (pLoopUnit->AI_getUnitAIType()==UNITAI_COUNTER))
                            {
                                countpatrolunits++;
                            }
                            if ((iFlag==1) && (pLoopUnit->AI_getUnitAIType()==UNITAI_WARWIZARD))
                            {
                                countpatrolunits++;
                            }
                        }
        ...
        m_aiPatrolNeeded[iFlag]=std::max(0,(patrolneeded-countpatrolunits));
    }
}

// runs once every turn in CvPlayer::doTurn
void CvCityAI::AI_calculateNeededCityDefenseProduction()
{
    for (int type=0;type<NUM_CITYDEFENSE_TIERS;type++)
    {

        if(AI_isCityDefenseProduction()==false)
        {
            m_aiCityDefenseProductionNeeded[type]=0;
            continue;
        }

        if (type==1)
        {
            if(GC.getCivilizationInfo(getCivilizationType()).getBestDefender1()==NO_UNITCLASS)
            {
                m_aiCityDefenseProductionNeeded[type]=0;
                continue;
            }

            if(GC.getCivilizationInfo(getCivilizationType()).getBestDefender2()!=NO_UNITCLASS)
            {
                if (canTrain((UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getCivilizationInfo(getCivilizationType()).getBestDefender2()),false,false,false,false))
                {
                    m_aiCityDefenseProductionNeeded[type]=0;
                    continue;
                }
            ...
            if(GC.getCivilizationInfo(getCivilizationType()).getBestDefender3()!=NO_UNITCLASS)
            {
                if (canTrain((UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getCivilizationInfo(getCivilizationType()).getBestDefender3()),false,false,false,false))
                {
                    m_aiCityDefenseProductionNeeded[type]=0;
                    continue;
                }
        ...
        for (CvCity* pLoopCity = GET_PLAYER(getOwnerINLINE()).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(getOwnerINLINE()).nextCity(&iLoop))
        {
            if (pLoopCity)
            {
                if(pLoopCity->AI_getCityDefenseProduction()==AI_getCityDefenseProduction())
                {
                    CLLNode<IDInfo>* pUnitNode;
                    CvUnit* pLoopUnit;
                    ...
                    while (pUnitNode != NULL)
                    {
                        ...
                        if (pLoopUnit!=NULL)
                        {
                            if(pLoopUnit->AI_getGroupflag()==GROUPFLAG_PERMDEFENSE && pLoopUnit->AI_getUnitAIType()==UNITAI_CITY_DEFENSE)
                            {
                                ...
                                {
                                    bvalid=true;
                                    if(pLoopUnit->getUnitClassType()==GC.getCivilizationInfo(getCivilizationType()).getBestDefender1())
                                        bvalid=false;
                                    if(pLoopUnit->getUnitClassType()==GC.getCivilizationInfo(getCivilizationType()).getBestDefender2())
                                        bvalid=false;
                                    if(pLoopUnit->getUnitClassType()==GC.getCivilizationInfo(getCivilizationType()).getBestDefender3())
                                        bvalid=false;
                                    if (bvalid)
                                        unitsneeded++;
                                }
        ...
        m_aiCityDefenseProductionNeeded[type]=unitsneeded;
    }
}

//How many more units the Patrol of the City could use
void CvCityAI::AI_calculateNeededCityPatrolProduction()
{
    for (int type=0;type<NUM_PATROL_TIERS;type++)
    {

        if(AI_isCityPatrolProduction()==false)
        {
            m_aiPatrolProductionNeeded[type]=0;
            continue;
        }

        if (type==1)
        {
            if(GC.getCivilizationInfo(getCivilizationType()).getBestPatrol1()==NO_UNITCLASS)
            {
                m_aiPatrolProductionNeeded[type]=0;
                continue;
            }

            if(GC.getCivilizationInfo(getCivilizationType()).getBestPatrol2()!=NO_UNITCLASS)
            {
                if (canTrain((UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getCivilizationInfo(getCivilizationType()).getBestPatrol2()),false,false,false,false))
                {
                    m_aiPatrolProductionNeeded[type]=0;
                    continue;
                }
            ...
            if(GC.getCivilizationInfo(getCivilizationType()).getBestPatrol3()!=NO_UNITCLASS)
            {
                if (canTrain((UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getCivilizationInfo(getCivilizationType()).getBestPatrol3()),false,false,false,false))
                {
                    m_aiPatrolProductionNeeded[type]=0;
                    continue;
                }
        ...
        for(CvUnit* pLoopUnit = GET_PLAYER(getOwnerINLINE()).firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = GET_PLAYER(getOwnerINLINE()).nextUnit(&iLoop))
        {
            if (pLoopUnit)
            {
                if(pLoopUnit->AI_getGroupflag()==GROUPFLAG_PATROL && pLoopUnit->AI_getUnitAIType()==UNITAI_COUNTER && pLoopUnit->getOriginPlot()==AI_getCityPatrolOrigin())
                {
                    bool bvalid;
                    if (type==1)
                    {
                        bvalid=true;
                        if(pLoopUnit->getUnitClassType()==GC.getCivilizationInfo(getCivilizationType()).getBestPatrol1())
                            bvalid=false;
                        if(pLoopUnit->getUnitClassType()==GC.getCivilizationInfo(getCivilizationType()).getBestPatrol2())
                            bvalid=false;
                        if(pLoopUnit->getUnitClassType()==GC.getCivilizationInfo(getCivilizationType()).getBestPatrol3())
                            bvalid=false;
                        if (bvalid)
                            unitsneeded++;
                    }
        ...
        m_aiPatrolProductionNeeded[type]=std::max(unitsneeded,AI_neededPatrol(0));
    }
}
...
void CvCityAI::AI_setCityPatrolOrigin(CvPlot* newpatrolorigin)
{
    if(newpatrolorigin==NULL)
    {
        m_iAI_CityPatrolgroupOrigin_X = -1;
        m_iAI_CityPatrolgroupOrigin_Y = -1;
    }
    ...
    {
        m_iAI_CityPatrolgroupOrigin_X = newpatrolorigin->getX_INLINE();
        m_iAI_CityPatrolgroupOrigin_Y = newpatrolorigin->getY_INLINE();
    }
}
...
void CvCityAI::AI_setCityDefenseProduction(CvPlot* newplot)
{
    if(newplot==NULL)
    {
        m_iAI_CityDefenseProduction_X = -1;
    ...
    {
        m_iAI_CityDefenseProduction_X = newplot->getX_INLINE();
        m_iAI_CityDefenseProduction_Y = newplot->getY_INLINE();
    }
}

CvPlot* CvCityAI::AI_getCityDefenseProduction()
{
    if (m_iAI_CityDefenseProduction_X == -1 && m_iAI_CityDefenseProduction_Y == -1)
    {
        return NULL;
    }
    else
    {
        return plotXY(m_iAI_CityDefenseProduction_X,m_iAI_CityDefenseProduction_Y,0,0);
    }
}

void CvCityAI::AI_CityChooseDefenseProduction()
{
    //Do we have a valid City for Production?
    if (AI_getCityDefenseProduction()!=NULL)
    {
        if(AI_getCityDefenseProduction()->getOwnerINLINE()==getOwnerINLINE() && AI_getCityDefenseProduction()->isCity())
        {
            return;
        }
    }
	...
	int Bestvalue=10;
	int iDX,iDY;
    // Is there a production city close by?
	for (iDX = -(iRange); iDX <= iRange; iDX++)
	{
		for (iDY = -(iRange); iDY <= iRange; iDY++)
		{
			...
			if (pLoopPlot != NULL)
			{
			    if(pLoopPlot->isCity() && pLoopPlot->getArea()==getArea() && pLoopPlot->getOwnerINLINE()==getOwnerINLINE())
			    {
			        if(pLoopPlot->getPlotCity()->AI_getCityDefenseProduction()!=NULL && pLoopPlot->getPlotCity()->AI_getCityDefenseProduction()==pLoopPlot)
			        {
                        value=stepDistance(getX_INLINE(), getY_INLINE(), pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE());
                        if (value<Bestvalue)
                        {
                            Bestvalue=value;
                            pBestCity=pLoopPlot;
	...
	if (pBestCity!=NULL)
	{
	    AI_setCityDefenseProduction(pBestCity);
	}
	//become a production City
	else
	{
	    AI_setCityDefenseProduction(plot());
	}
    return;
}

int CvCityAI::AI_neededCityDefenseProduction(int type)
{
    return m_aiCityDefenseProductionNeeded[type];
}

bool CvCityAI::AI_isCityDefenseProduction()
{
    if (AI_getCityDefenseProduction()==NULL)
    {
        return false;
    }
    if (AI_getCityDefenseProduction()==plot())
    {
        return true;
    }
...
void CvCityAI::AI_setCityPatrolProduction(CvPlot* newplot)
{
    if(newplot==NULL)
    {
        m_iAI_CityPatrolProduction_X = -1;
        m_iAI_CityPatrolProduction_Y = -1;
    }
    else
    {
        m_iAI_CityPatrolProduction_X = newplot->getX_INLINE();
        m_iAI_CityPatrolProduction_Y = newplot->getY_INLINE();
    }
}

CvPlot* CvCityAI::AI_getCityPatrolProduction()
{
    if (m_iAI_CityPatrolProduction_X == -1 && m_iAI_CityPatrolProduction_Y == -1)
    {
        return NULL;
    }
    else
    {
        return plotXY(m_iAI_CityPatrolProduction_X,m_iAI_CityPatrolProduction_Y,0,0);
    }
}


void CvCityAI::AI_CityChoosePatrolProduction()
{
    //Capital always is a Patrol Production City to get things started
    if (isCapital())
    {
        AI_setCityPatrolProduction(plot());
        return;
    }

    //Do we already have a Patrol group?
    if (AI_getCityPatrolOrigin()==NULL)
    {
        return;
    }
    //Do we have a valid City for Production?
    if (AI_getCityPatrolProduction()!=NULL)
    {
        if(AI_getCityPatrolProduction()->getOwnerINLINE()==getOwnerINLINE() && AI_getCityPatrolProduction()->isCity())
        {
            return;
        }
    }
	...
	for (CvCity* pLoopCity = GET_PLAYER(getOwnerINLINE()).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(getOwnerINLINE()).nextCity(&iLoop))
	{
		if (pLoopCity && pLoopCity!=this)
		{
		    if(pLoopCity->AI_getCityPatrolOrigin()==AI_getCityPatrolOrigin())
			{
			    if(pLoopCity->getArea()==getArea())
			    {
			        if(pLoopCity->AI_getCityPatrolProduction()!=NULL)
			        {
                        AI_setCityPatrolProduction(pLoopCity->plot());
			        }
			    }
			}
		}
...
//How many more units the Patrol of the City could use
int CvCityAI::AI_neededCityPatrolProduction(int type)
{
    return m_aiPatrolProductionNeeded[type];
}

bool CvCityAI::AI_isCityPatrolProduction()
{
    if (AI_getCityPatrolProduction()==NULL)
    {
        return false;
    }
    if (AI_getCityPatrolProduction()==plot())
    {
        return true;
    }
