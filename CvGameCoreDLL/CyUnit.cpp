...
{
	return m_pUnit ? m_pUnit->getDamageTypeCombat((DamageTypes)eDamageType) : -1;
}
...
{
	return m_pUnit ? m_pUnit->getDamageTypeResist((DamageTypes)eDamageType) : -1;
}
//<<<<Unofficial Bug Fix: End Add

//>>>>Unofficial Bug Fix: Added by Denev 2010/02/21
bool CyUnit::isMechUnit() const
{
	return m_pUnit ? m_pUnit->isMechUnit() : false;
}

bool CyUnit::isAvatarOfCivLeader() const
{
	return m_pUnit ? m_pUnit->isAvatarOfCivLeader() : false;
}

void CyUnit::setAvatarOfCivLeader(bool bNewValue)
{
	if (m_pUnit)
		m_pUnit->setAvatarOfCivLeader(bNewValue);
}
//<<<<Unofficial Bug Fix: End Add
