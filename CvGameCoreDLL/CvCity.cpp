...
#include "CyCity.h"
//>>>>Unofficial Bug Fix: Added by Denev 2010/02/21
#include "CyUnit.h"
...
/*************************************************************************************************/
	...
	if (pPlot->getCulture(eOwner) < GC.defines.iFREE_CITY_CULTURE)
	{
		pPlot->setCulture(eOwner, GC.defines.iFREE_CITY_CULTURE, bBumpUnits, false);
	}
	...
	for (iI = 0; iI < NUM_DIRECTION_TYPES; iI++)
	{
		...
		{
			if (pAdjacentPlot->getCulture(eOwner) < GC.defines.iFREE_CITY_ADJACENT_CULTURE)
			{
				pAdjacentPlot->setCulture(eOwner, GC.defines.iFREE_CITY_ADJACENT_CULTURE, bBumpUnits, false);
			}
	...
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		...
		{
			pPlot->changeAdjacentSight((TeamTypes)iI, GC.defines.iPLOT_VISIBILITY_RANGE, true, NULL, false);
		}
	}
...
/************************************************************************************************/
	...
	changePopulation(GC.defines.iINITIAL_CITY_POPULATION + GC.getEraInfo(GC.getGameINLINE().getStartEra()).getFreePopulation());

	changeAirUnitCapacity(GC.defines.iCITY_AIR_UNIT_CAPACITY);
	...
	if (GC.getGameINLINE().isFinalInitialized())
	{
		if (kPlayer.getNumCities() == 1)
		{
			...
			if (!isHuman())
			{
				changeOverflowProduction(GC.defines.iINITIAL_AI_CITY_PRODUCTION, 0);
			}
		}
	}
...
//<<<<Unofficial Bug Fix: End Modify
		...
		FAssertMsg((0 < GC.defines.iMAX_TRADE_ROUTES),  "GC.getMAX_TRADE_ROUTES() is not greater than zero but an array is being allocated in CvCity::reset");
		m_paTradeCities = new IDInfo[GC.defines.iMAX_TRADE_ROUTES];
		for (iI = 0; iI < GC.defines.iMAX_TRADE_ROUTES; iI++)
		{
...
/************************************************************************************************/
	...
	pPlot->setImprovementType((ImprovementTypes)(GC.defines.iRUINS_IMPROVEMENT));
	...
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		...
		{
			pPlot->changeAdjacentSight((TeamTypes)iI, GC.defines.iPLOT_VISIBILITY_RANGE, false, NULL, false);
		}
	}
...
/************************************************************************************************/
	...
	else if ((getPopulation() >= GC.defines.iWE_LOVE_THE_KING_POPULATION_MIN_POPULATION) && (GC.getGameINLINE().getSorenRandNum(GC.defines.iWE_LOVE_THE_KING_RAND, "Do We Love The King?") < getPopulation()))
	{
...
bool CvCity::isWorldWondersMaxed() const
{
	...
	if (GC.defines.iMAX_WORLD_WONDERS_PER_CITY == -1)
	{
		return false;
	}

	if (getNumWorldWonders() >= GC.defines.iMAX_WORLD_WONDERS_PER_CITY)
	{
		return true;
	}
...
bool CvCity::isTeamWondersMaxed() const
{
	...
	if (GC.defines.iMAX_TEAM_WONDERS_PER_CITY == -1)
	{
		return false;
	}

	if (getNumTeamWonders() >= GC.defines.iMAX_TEAM_WONDERS_PER_CITY)
	{
		return true;
	}
...
{
	int iMaxNumWonders = (GC.getGameINLINE().isOption(GAMEOPTION_ONE_CITY_CHALLENGE) && isHuman()) ? GC.defines.iMAX_NATIONAL_WONDERS_PER_CITY_FOR_OCC : GC.defines.iMAX_NATIONAL_WONDERS_PER_CITY;
...
bool CvCity::isBuildingsMaxed() const
{
	...
	if (GC.defines.iMAX_BUILDINGS_PER_CITY == -1)
	{
		return false;
	}

	if (getNumBuildings() >= GC.defines.iMAX_BUILDINGS_PER_CITY)
	{
		return true;
	}
...
//FfH: Added by Kael 09/26/2007
    ...
    else
    {
        ...
        if (kUnitInfo.getUpgradeCiv() == NO_CIVILIZATION)
        {
        ...
        {
            if (GC.getCivilizationInfo((CivilizationTypes)kUnitInfo.getUpgradeCiv()).getCivilizationUnits(eUnitClass) != eUnit)
            {
                return false;
            }
        }
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.defines.iUSE_CANCREATE_CALLBACK==1)
	{
        CyCity* pyCity = new CyCity((CvCity*)this);
        CyArgsList argsList;
        argsList.add(gDLL->getPythonIFace()->makePythonObject(pyCity));	// pass in city class
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.defines.iUSE_CANMAINTAIN_CALLBACK==1)
	{
        CyCity* pyCity = new CyCity((CvCity*)this);
        CyArgsList argsList;
        argsList.add(gDLL->getPythonIFace()->makePythonObject(pyCity));	// pass in city class
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.defines.iUSE_CANNOTMAINTAIN_CALLBACK==1)
	{
        CyCity* pyCity = new CyCity((CvCity*)this);
        CyArgsList argsList2; // XXX
        argsList2.add(gDLL->getPythonIFace()->makePythonObject(pyCity));	// pass in city class
...
int CvCity::getProductionModifier(UnitTypes eUnit) const
{
	...
	iMultiplier += getDomainProductionModifier((DomainTypes)(kUnitInfo.getDomainType()));
	...
	for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
	{
		...
		{
			iMultiplier += kUnitInfo.getBonusProductionModifier(iI);
		}
	}
...
bool CvCity::hurryOverflow(HurryTypes eHurry, int* iProduction, int* iGold, bool bCountThisTurn) const
{
	...
	if (isProductionUnit())
	{
		...
		iGoldPercent = GC.defines.iMAXED_UNIT_GOLD_PERCENT;
	}
	else if (isProductionBuilding())
	{
		...
		iGoldPercent = GC.defines.iMAXED_BUILDING_GOLD_PERCENT;
	}
	else if (isProductionProject())
	{
		...
		iGoldPercent = GC.defines.iMAXED_PROJECT_GOLD_PERCENT;
	}
	else
	{
		return false;
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.defines.iUSE_CONSCRIPTUNIT_CALLBACK==1)
	{
...
int CvCity::getConscriptPopulation() const
{
	...
	if (GC.defines.iCONSCRIPT_POPULATION_PER_COST == 0)
	{
		return 0;
	}

	return std::max(1, ((GC.getUnitInfo(eConscriptUnit).getProductionCost()) / GC.defines.iCONSCRIPT_POPULATION_PER_COST));
}
...
int CvCity::conscriptMinCityPopulation() const
{
	...
	iPopulation = GC.defines.iCONSCRIPT_MIN_CITY_POPULATION;
...
int CvCity::flatConscriptAngerLength() const
{
	...
	iAnger = GC.defines.iCONSCRIPT_ANGER_DIVISOR;
...
bool CvCity::canConscript() const
{
	...
	if (plot()->calculateTeamCulturePercent(getTeam()) < GC.defines.iCONSCRIPT_MIN_CULTURE_PERCENT)
	{
		return false;
	}
...
void CvCity::processBuilding(BuildingTypes eBuilding, int iChange, bool bObsolete)
{
	...
	if (!(GET_TEAM(getTeam()).isObsoleteBuilding(eBuilding)) || bObsolete)
	{
		if (iChange > 0)
		{
			CorporationTypes eCorporation = (CorporationTypes)kBuildingInfo.getFoundsCorporation();
		...
		if (kBuildingInfo.getNoBonus() != NO_BONUS)
		{
			changeNoBonusCount(((BonusTypes)(kBuildingInfo.getNoBonus())), iChange);
		}

		if (kBuildingInfo.getFreeBonus() != NO_BONUS)
		{
			changeFreeBonus(((BonusTypes)(kBuildingInfo.getFreeBonus())), (GC.getGameINLINE().getNumFreeBonuses(eBuilding) * iChange));
		}

//FfH: Added by Kael 08/04/2007
        changeCrime(kBuildingInfo.getCrime() * iChange);
        changeUnhappyProduction(iChange * kBuildingInfo.isUnhappyProduction());
		if (kBuildingInfo.getFreeBonus2() != NO_BONUS)
		{
			changeFreeBonus(((BonusTypes)(kBuildingInfo.getFreeBonus2())), (GC.getGameINLINE().getNumFreeBonuses(eBuilding) * iChange));
		}
		if (kBuildingInfo.getFreeBonus3() != NO_BONUS)
		{
			changeFreeBonus(((BonusTypes)(kBuildingInfo.getFreeBonus3())), (GC.getGameINLINE().getNumFreeBonuses(eBuilding) * iChange));
		}

        GC.getGameINLINE().changeGlobalCounter(kBuildingInfo.getModifyGlobalCounter());
        if (kBuildingInfo.getResistMagic() != 0)
        {
            changeResistMagic(kBuildingInfo.getResistMagic() * iChange);
        }
        if (kBuildingInfo.getPlotRadius())
        {
            if (iChange > 0)
            {
                setPlotRadius(kBuildingInfo.getPlotRadius());
            }
            else
            {
                setPlotRadius(2);
...
//FfH: End Add
		...
		if (kBuildingInfo.getFreePromotion() != NO_PROMOTION)
		{
			changeFreePromotionCount(((PromotionTypes)(kBuildingInfo.getFreePromotion())), iChange);
		}

		changeEspionageDefenseModifier(kBuildingInfo.getEspionageDefenseModifier() * iChange);
		changeGreatPeopleRateModifier(kBuildingInfo.getGreatPeopleRateModifier() * iChange);
		changeFreeExperience(kBuildingInfo.getFreeExperience() * iChange);
		changeMaxFoodKeptPercent(kBuildingInfo.getFoodKept() * iChange);
		changeMaxAirlift(kBuildingInfo.getAirlift() * iChange);
		changeAirModifier(kBuildingInfo.getAirModifier() * iChange);
		changeAirUnitCapacity(kBuildingInfo.getAirUnitCapacity() * iChange);
		changeNukeModifier(kBuildingInfo.getNukeModifier() * iChange);
		changeFreeSpecialist(kBuildingInfo.getFreeSpecialist() * iChange);
		changeMaintenanceModifier(kBuildingInfo.getMaintenanceModifier() * iChange);
		changeWarWearinessModifier(kBuildingInfo.getWarWearinessModifier() * iChange);
		changeHurryAngerModifier(kBuildingInfo.getHurryAngerModifier() * iChange);
		changeHealRate(kBuildingInfo.getHealRateChange() * iChange);
		if (kBuildingInfo.getHealth() > 0)
		{
			changeBuildingGoodHealth(kBuildingInfo.getHealth() * iChange);
		}
		else
		{
			changeBuildingBadHealth(kBuildingInfo.getHealth() * iChange);
		}
		if (kBuildingInfo.getHappiness() > 0)
		{
			changeBuildingGoodHappiness(kBuildingInfo.getHappiness() * iChange);
		}
		else
		{
			changeBuildingBadHappiness(kBuildingInfo.getHappiness() * iChange);
		}
		if (kBuildingInfo.getReligionType() != NO_RELIGION)
		{
		...
		changeForeignTradeRouteModifier(kBuildingInfo.getForeignTradeRouteModifier() * iChange);
		changePowerCount(((kBuildingInfo.isPower()) ? iChange : 0), kBuildingInfo.isDirtyPower());
		changeGovernmentCenterCount((kBuildingInfo.isGovernmentCenter()) ? iChange : 0);
		changeNoUnhappinessCount((kBuildingInfo.isNoUnhappiness()) ? iChange : 0);
		changeNoUnhealthyPopulationCount((kBuildingInfo.isNoUnhealthyPopulation()) ? iChange : 0);
		changeBuildingOnlyHealthyCount((kBuildingInfo.isBuildingOnlyHealthy()) ? iChange : 0);

		for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
		{
			changeSeaPlotYield(((YieldTypes)iI), (kBuildingInfo.getSeaPlotYieldChange(iI) * iChange));
			changeRiverPlotYield(((YieldTypes)iI), (kBuildingInfo.getRiverPlotYieldChange(iI) * iChange));
			changeBaseYieldRate(((YieldTypes)iI), ((kBuildingInfo.getYieldChange(iI) + getBuildingYieldChange((BuildingClassTypes)kBuildingInfo.getBuildingClassType(), (YieldTypes)iI))* iChange));
			changeYieldRateModifier(((YieldTypes)iI), (kBuildingInfo.getYieldModifier(iI) * iChange));
			changePowerYieldRateModifier(((YieldTypes)iI), (kBuildingInfo.getPowerYieldModifier(iI) * iChange));
		}

		for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
		{
			changeCommerceRateModifier(((CommerceTypes)iI), (kBuildingInfo.getCommerceModifier(iI) * iChange));
			changeCommerceHappinessPer(((CommerceTypes)iI), (kBuildingInfo.getCommerceHappiness(iI) * iChange));
		}

		for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
		{
			changeReligionInfluence(((ReligionTypes)iI), (kBuildingInfo.getReligionChange(iI) * iChange));
		}

		for (iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
		{
			changeMaxSpecialistCount(((SpecialistTypes)iI), kBuildingInfo.getSpecialistCount(iI) * iChange);
			changeFreeSpecialistCount(((SpecialistTypes)iI), kBuildingInfo.getFreeSpecialistCount(iI) * iChange);
		}

		for (iI = 0; iI < GC.getNumImprovementInfos(); ++iI)
		{
			changeImprovementFreeSpecialists((ImprovementTypes)iI, kBuildingInfo.getImprovementFreeSpecialist(iI) * iChange);
		}
		...
		for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
		{
			if (hasBonus((BonusTypes)iI))
			{
				if (kBuildingInfo.getBonusHealthChanges(iI) > 0)
				{
					changeBonusGoodHealth(kBuildingInfo.getBonusHealthChanges(iI) * iChange);
				}
				else
				{
					changeBonusBadHealth(kBuildingInfo.getBonusHealthChanges(iI) * iChange);
				}
				if (kBuildingInfo.getBonusHappinessChanges(iI) > 0)
				{
					changeBonusGoodHappiness(kBuildingInfo.getBonusHappinessChanges(iI) * iChange);
				}
				else
				{
					changeBonusBadHappiness(kBuildingInfo.getBonusHappinessChanges(iI) * iChange);
				}

				if (kBuildingInfo.getPowerBonus() == iI)
				{
					changePowerCount(iChange, kBuildingInfo.isDirtyPower());
				}

				for (iJ = 0; iJ < NUM_YIELD_TYPES; iJ++)
				{
					changeBonusYieldRateModifier(((YieldTypes)iJ), (kBuildingInfo.getBonusYieldModifier(iI, iJ) * iChange));
				}
			}
		}
		...
		for (iI = 0; iI < GC.getNumUnitCombatInfos(); iI++)
		{
			changeUnitCombatFreeExperience(((UnitCombatTypes)iI), kBuildingInfo.getUnitCombatFreeExperience(iI) * iChange);
		}

		for (iI = 0; iI < NUM_DOMAIN_TYPES; iI++)
		{
			changeDomainFreeExperience(((DomainTypes)iI), kBuildingInfo.getDomainFreeExperience(iI) * iChange);
			changeDomainProductionModifier(((DomainTypes)iI), kBuildingInfo.getDomainProductionModifier(iI) * iChange);
		}

		updateExtraBuildingHappiness();
		updateExtraBuildingHealth();

		GET_PLAYER(getOwnerINLINE()).changeAssets(kBuildingInfo.getAssetValue() * iChange);

		area()->changePower(getOwnerINLINE(), (kBuildingInfo.getPowerValue() * iChange));
		GET_PLAYER(getOwnerINLINE()).changePower(kBuildingInfo.getPowerValue() * iChange);

		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).getTeam() == getTeam())
			{
				if (kBuildingInfo.isTeamShare() || (iI == getOwnerINLINE()))
				{
	...
	if (!bObsolete)
	{
		...
		if (kBuildingInfo.getGreatPeopleUnitClass() != NO_UNITCLASS)
		{
			...
			{
				changeGreatPeopleUnitRate(eGreatPeopleUnit, kBuildingInfo.getGreatPeopleRateChange() * iChange);
			}
		}

		GET_TEAM(getTeam()).changeBuildingClassCount((BuildingClassTypes)kBuildingInfo.getBuildingClassType(), iChange);
		GET_PLAYER(getOwnerINLINE()).changeBuildingClassCount((BuildingClassTypes)kBuildingInfo.getBuildingClassType(), iChange);

		GET_PLAYER(getOwnerINLINE()).changeWondersScore(getWonderScore((BuildingClassTypes)(kBuildingInfo.getBuildingClassType())) * iChange);
	}
...
int CvCity::getNoMilitaryPercentAnger() const
{
	...
	{
		iAnger += GC.defines.iNO_MILITARY_PERCENT_ANGER;
	}
...
int CvCity::getCulturePercentAnger() const
{
		...
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if (GET_PLAYER((PlayerTypes)iI).getTeam() != getTeam())
			{
				...
				if (iCulture > 0)
				{
					...
					{
						iCulture *= std::max(0, (GC.defines.iAT_WAR_CULTURE_ANGER_MODIFIER + 100));
						iCulture /= 100;
					}
	...
	return ((GC.defines.iCULTURE_PERCENT_ANGER * iAngryCulture) / iTotalCulture);
}
...
int CvCity::getReligionPercentAnger() const
{
	...
	iAnger = GC.defines.iRELIGION_PERCENT_ANGER;
...
int CvCity::getHurryPercentAnger(int iExtra) const
{
	...
	return ((((((getHurryAngerTimer() - 1) / flatHurryAngerLength()) + 1) * GC.defines.iHURRY_POP_ANGER * GC.getPERCENT_ANGER_DIVISOR()) / std::max(1, getPopulation() + iExtra)) + 1);
}
...
int CvCity::getConscriptPercentAnger(int iExtra) const
{
	...
	return ((((((getConscriptAngerTimer() - 1) / flatConscriptAngerLength()) + 1) * GC.defines.iCONSCRIPT_POP_ANGER * GC.getPERCENT_ANGER_DIVISOR()) / std::max(1, getPopulation() + iExtra)) + 1);
}
...
int CvCity::getDefyResolutionPercentAnger(int iExtra) const
{
	...
	return ((((((getDefyResolutionAngerTimer() - 1) / flatDefyResolutionAngerLength()) + 1) * GC.defines.iDEFY_RESOLUTION_POP_ANGER * GC.getPERCENT_ANGER_DIVISOR()) / std::max(1, getPopulation() + iExtra)) + 1);
}
...
int CvCity::getRevRequestPercentAnger(int iExtra) const
{
	...
	iAnger = GC.defines.iHURRY_ANGER_DIVISOR;
	iAnger *= GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getHurryConscriptAngerPercent();
	iAnger /= 100;

	iAnger = std::max(1, iAnger);

	iAngerPercent = 2*((((((getRevRequestAngerTimer() - 1) / iAnger) + 1) * GC.defines.iHURRY_POP_ANGER * GC.getPERCENT_ANGER_DIVISOR()) / std::max(1, getPopulation() + iExtra)) + 1);
...
int CvCity::getRevSuccessHappiness() const
{
	...
	iHappy = GC.defines.iHURRY_ANGER_DIVISOR;
	iHappy *= GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getHurryConscriptAngerPercent();
	iHappy /= 100;

	iHappy = std::max(1, iHappy);

	iHappyPercent = 2*((((((getRevSuccessTimer() - 1) / iHappy) + 1) * GC.defines.iHURRY_POP_ANGER * GC.getPERCENT_ANGER_DIVISOR()) / std::max(1, getPopulation())) + 1);
...
int CvCity::getVassalHappiness() const
{
	...
	for (int i = 0; i < MAX_TEAMS; i++)
	{
		if (getTeam() != i)
		{
			...
			{
				iHappy += GC.defines.iVASSAL_HAPPINESS;
			}
		}
	}
...
int CvCity::getVassalUnhappiness() const
{
	...
	for (int i = 0; i < MAX_TEAMS; i++)
	{
		if (getTeam() != i)
		{
			...
			{
				iUnhappy += GC.defines.iVASSAL_HAPPINESS;
			}
		}
	}
...
/*************************************************************************************************/
	...
	if (getHappinessTimer() > 0)
	{
		iHappiness += GC.defines.iTEMP_HAPPY;
	}
...
int CvCity::getHurryCostModifier(int iBaseModifier, int iProduction, bool bIgnoreNew) const
{
	...
	{
		iModifier *= std::max(0, (GC.defines.iNEW_HURRY_MODIFIER + 100));
		iModifier /= 100;
	}
...
int CvCity::cultureStrength(PlayerTypes ePlayer) const
{
	...
	iStrength *= std::max(0, (((GC.defines.iREVOLT_TOTAL_CULTURE_MODIFIER * (plot()->getCulture(ePlayer) - plot()->getCulture(getOwnerINLINE()) + 1)) / (plot()->getCulture(ePlayer) + 1)) + 100));
	iStrength /= 100;
	...
	if (GET_PLAYER(ePlayer).getStateReligion() != NO_RELIGION)
	{
		...
		{
			iStrength *= std::max(0, (GC.defines.iREVOLT_OFFENSE_STATE_RELIGION_MODIFIER + 100));
			iStrength /= 100;
		}
	}
	...
	if (GET_PLAYER(getOwnerINLINE()).getStateReligion() != NO_RELIGION)
	{
		if (isHasReligion(GET_PLAYER(getOwnerINLINE()).getStateReligion()))
		{
			iStrength *= std::max(0, (GC.defines.iREVOLT_DEFENSE_STATE_RELIGION_MODIFIER + 100));
			iStrength /= 100;
		}
	}
...
int CvCity::getTotalGreatPeopleRateModifier() const
{
	...
	{
		iModifier += GC.defines.iGOLDEN_AGE_GREAT_PEOPLE_MODIFIER;
	}
...
//FfH: End Add
	...
	for (pLoopCity = GET_PLAYER(getOwnerINLINE()).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(getOwnerINLINE()).nextCity(&iLoop))
	{
		iTempMaintenance = 100 * (GC.defines.iMAX_DISTANCE_CITY_MAINTENANCE * plotDistance(getX_INLINE(), getY_INLINE(), pLoopCity->getX_INLINE(), pLoopCity->getY_INLINE()));
...
void CvCity::updateFreshWaterHealth()
{
	...
	{
		if (GC.defines.iFRESH_WATER_HEALTH_CHANGE > 0)
		{
			iNewGoodHealth += GC.defines.iFRESH_WATER_HEALTH_CHANGE;
		}
		else
		{
			iNewBadHealth += GC.defines.iFRESH_WATER_HEALTH_CHANGE;
		}
	}
...
void CvCity::updatePowerHealth()
{
	...
	if (isPower())
	{
		int iPowerHealth = GC.defines.iPOWER_HEALTH_CHANGE;
		if (iPowerHealth > 0)
		{
	...
	if (isDirtyPower())
	{
		int iDirtyPowerHealth = GC.defines.iDIRTY_POWER_HEALTH_CHANGE;
...
int CvCity::getBuildingHappiness(BuildingTypes eBuilding) const
{
	...
	iHappiness = kBuildingInfo.getHappiness();

	if (kBuildingInfo.getReligionType() != NO_RELIGION)
	{
		if (kBuildingInfo.getReligionType() == GET_PLAYER(getOwnerINLINE()).getStateReligion())
		{
			iHappiness += kBuildingInfo.getStateReligionHappiness();
		}
	}
	...
	for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		iHappiness += ((kBuildingInfo.getCommerceHappiness(iI) * GET_PLAYER(getOwnerINLINE()).getCommercePercent((CommerceTypes)iI)) / 100);
	}

	iHappiness += getBuildingHappyChange((BuildingClassTypes)kBuildingInfo.getBuildingClassType());
...
int CvCity::getAdditionalHealthByBuilding(BuildingTypes eBuilding, int& iGood, int& iBad) const
{
	...
	if (kBuilding.isPower() || kBuilding.isAreaCleanPower() || (kBuilding.getPowerBonus() != NO_BONUS && hasBonus((BonusTypes)kBuilding.getPowerBonus())))
	{
		...
		if (!isPower())
		{
			addGoodOrBad(GC.defines.iPOWER_HEALTH_CHANGE, iGood, iBad);

			// adding dirty power
			if (kBuilding.isDirtyPower())
			{
				addGoodOrBad(GC.defines.iDIRTY_POWER_HEALTH_CHANGE, iGood, iBad);
			}
		}
		else
		{
			...
			{
				subtractGoodOrBad(GC.defines.iDIRTY_POWER_HEALTH_CHANGE, iGood, iBad);
			}
		}
	}
...
int CvCity::flatDefyResolutionAngerLength() const
{
	...
	iAnger = GC.defines.iDEFY_RESOLUTION_ANGER_DIVISOR;
...
{
	return (getTeam() == eTeam ? m_iAirUnitCapacity : GC.defines.iCITY_AIR_UNIT_CAPACITY);
}
...
/* original BTS
	if (!isOccupation())
*/
	int iMaxOccupationTimer = GC.defines.iBASE_OCCUPATION_TURNS + ((getHighestPopulation() * GC.defines.iOCCUPATION_TURNS_POPULATION_PERCENT) / 100);
...
int CvCity::totalTradeModifier(CvCity* pOtherCity) const
{
	...
	{
		iModifier += GC.defines.iCAPITAL_TRADE_MODIFIER;
	}
	...
	if (NULL != pOtherCity)
	{
		...
		{
			iModifier += GC.defines.iOVERSEAS_TRADE_MODIFIER;
		}
...
{
	return std::max(0, (getPopulation() + GC.defines.iOUR_POPULATION_TRADE_MODIFIER_OFFSET) * GC.defines.iOUR_POPULATION_TRADE_MODIFIER);
}
...
int CvCity::getPeaceTradeModifier(TeamTypes eTeam) const
{
	...
	int iPeaceTurns = std::min(GC.defines.iFOREIGN_TRADE_FULL_CREDIT_PEACE_TURNS, GET_TEAM(getTeam()).AI_getAtPeaceCounter(eTeam));

	if (GC.getGameINLINE().getElapsedGameTurns() <= iPeaceTurns)
	{
		return GC.defines.iFOREIGN_TRADE_MODIFIER;
	}

	return ((GC.defines.iFOREIGN_TRADE_MODIFIER * iPeaceTurns) / std::max(1, GC.defines.iFOREIGN_TRADE_FULL_CREDIT_PEACE_TURNS));
}

int CvCity::getBaseTradeProfit(CvCity* pCity) const
{
	int iProfit = std::min(pCity->getPopulation() * GC.defines.iTHEIR_POPULATION_TRADE_PERCENT, plotDistance(getX_INLINE(), getY_INLINE(), pCity->getX_INLINE(), pCity->getY_INLINE()) * GC.getWorldInfo(GC.getMapINLINE().getWorldSize()).getTradeProfitPercent());

	iProfit *= GC.defines.iTRADE_PROFIT_PERCENT;
	iProfit /= 100;
...
int CvCity::getBuildingCommerceByBuilding(CommerceTypes eIndex, BuildingTypes eBuilding) const
{
	...
	{
		CvBuildingInfo& kBuildingInfo = GC.getBuildingInfo(eBuilding);
		if (!(kBuildingInfo.isCommerceChangeOriginalOwner(eIndex)) || (getBuildingOriginalOwner(eBuilding) == getOwnerINLINE()))
		{
			iCommerce += kBuildingInfo.getObsoleteSafeCommerceChange(eIndex) * getNumBuilding(eBuilding);

			if (getNumActiveBuilding(eBuilding) > 0)
			{
				iCommerce += (kBuildingInfo.getCommerceChange(eIndex) + getBuildingCommerceChange((BuildingClassTypes)kBuildingInfo.getBuildingClassType(), eIndex)) * getNumActiveBuilding(eBuilding);
				...
				if (kBuildingInfo.getGlobalReligionCommerce() != NO_RELIGION)
				{
					iCommerce += (GC.getReligionInfo((ReligionTypes)(kBuildingInfo.getGlobalReligionCommerce())).getGlobalReligionCommerce(eIndex) * GC.getGameINLINE().countReligionLevels((ReligionTypes)(kBuildingInfo.getGlobalReligionCommerce()))) * getNumActiveBuilding(eBuilding);
				}

				if (kBuildingInfo.getGlobalCorporationCommerce() != NO_CORPORATION)
				{
					iCommerce += (GC.getCorporationInfo((CorporationTypes)(kBuildingInfo.getGlobalCorporationCommerce())).getHeadquarterCommerce(eIndex) * GC.getGameINLINE().countCorporationLevels((CorporationTypes)(kBuildingInfo.getGlobalCorporationCommerce()))) * getNumActiveBuilding(eBuilding);
				}
			}
/*************************************************************************************************/
...
/*************************************************************************************************/
/** Orig
			if ((kBuildingInfo.getCommerceChangeDoubleTime(eIndex) != 0) &&
				(getBuildingOriginalTime(eBuilding) != MIN_INT) &&
				((GC.getGameINLINE().getGameTurnYear() - getBuildingOriginalTime(eBuilding)) >= kBuildingInfo.getCommerceChangeDoubleTime(eIndex)))
			{
				return (iCommerce * 2);
			}
**/
            int neededturn=kBuildingInfo.getCommerceChangeDoubleTime(eIndex)*GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getConstructPercent();
			neededturn/=100;

			if ((kBuildingInfo.getCommerceChangeDoubleTime(eIndex) != 0) &&
				(getBuildingOriginalTime(eBuilding) != MIN_INT) &&
				((GC.getGameINLINE().getGameTurn() - getBuildingOriginalTime(eBuilding)) >= neededturn))
			{
				return (iCommerce * 2);
			}
...
int CvCity::getRevoltTestProbability() const
{
	...
	return ((GC.defines.iREVOLT_TEST_PROB * (100 - iBestModifier)) / 100);
}
...
bool CvCity::isBuildingProductionDecay(BuildingTypes eIndex) const																			 
{
	...
	return GET_PLAYER( getOwnerINLINE() ).getDisableProduction() == 0 // lfgr 09/2019: No decay while stasis
			...
			&& 100 * getBuildingProductionTime(eIndex) >= GC.defines.iBUILDING_PRODUCTION_DECAY_TIME * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getConstructPercent();
}
...
int CvCity::getBuildingProductionDecay(BuildingTypes eIndex) const																			 
{
	...
	return (iProduction * (100 - GC.defines.iBUILDING_PRODUCTION_DECAY_PERCENT) + iGameSpeedPercent - 1) / iGameSpeedPercent;
}
...
int CvCity::getBuildingProductionDecayTurns(BuildingTypes eIndex) const																			 
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < GC.getNumBuildingInfos(), "eIndex expected to be < GC.getNumBuildingInfos()");
	return std::max(0, (GC.defines.iBUILDING_PRODUCTION_DECAY_TIME * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getConstructPercent() + 99) / 100 - getBuildingProductionTime(eIndex)) + 1;
}
...
bool CvCity::isUnitProductionDecay(UnitTypes eIndex) const																			 
{
	...
	return GET_PLAYER( getOwnerINLINE() ).getDisableProduction() == 0 // lfgr 09/2019: No decay while stasis
			...
			&& 100 * getUnitProductionTime(eIndex) >= GC.defines.iUNIT_PRODUCTION_DECAY_TIME * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getTrainPercent();
}
...
int CvCity::getUnitProductionDecay(UnitTypes eIndex) const																			 
{
	...
	return (iProduction * (100 - GC.defines.iUNIT_PRODUCTION_DECAY_PERCENT) + iGameSpeedPercent - 1) / iGameSpeedPercent;
}
...
int CvCity::getUnitProductionDecayTurns(UnitTypes eIndex) const																			 
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < GC.getNumUnitInfos(), "eIndex expected to be < GC.getNumUnitInfos()");
	return std::max(0, (GC.defines.iUNIT_PRODUCTION_DECAY_TIME * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getTrainPercent() + 99) / 100 - getUnitProductionTime(eIndex)) + 1;
}
// BUG - Production Decay - end
			...
			else
			{
				for (iI = 0; iI < -(iChange); iI++)
				{
					if (getSpecialistCount(eIndex) > 0)
					{
						...
						if ((eIndex != GC.defines.iDEFAULT_SPECIALIST) && (GC.defines.iDEFAULT_SPECIALIST != NO_SPECIALIST))
						{
...
{
	return (((getSpecialistCount(eIndex) + iExtra) <= getMaxSpecialistCount(eIndex)) || GET_PLAYER(getOwnerINLINE()).isSpecialistValid(eIndex) || (eIndex == GC.defines.iDEFAULT_SPECIALIST));
}
						...
						for (iI = 0; iI < MAX_PLAYERS; iI++)
						{
							if (GET_PLAYER((PlayerTypes)iI).isAlive())
							{
								if (isRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false))
								{
									...
									gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_WONDER_BUILDING_BUILD", MESSAGE_TYPE_MAJOR_EVENT, GC.getBuildingInfo(eIndex).getArtInfo()->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_PLAYER_LIGHT_ORANGE_TEXT"), getX_INLINE(), getY_INLINE(), true, true);
								}
								else
								{
									szBuffer = gDLL->getText("TXT_KEY_MISC_WONDER_COMPLETED_UNKNOWN", GC.getBuildingInfo(eIndex).getTextKeyWide());
									gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_WONDER_BUILDING_BUILD", MESSAGE_TYPE_MAJOR_EVENT, GC.getBuildingInfo(eIndex).getArtInfo()->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_PLAYER_LIGHT_ORANGE_TEXT"));
								}
							}
						}
					}
...
//									gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.defines.iEVENT_MESSAGE_TIME_LONG, szBuffer, GC.getReligionInfo(eIndex).getSound(), MESSAGE_TYPE_MAJOR_EVENT, GC.getReligionInfo(eIndex).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), getX_INLINE(), getY_INLINE(), bArrows, bArrows);
				...
				if (GET_PLAYER((PlayerTypes)iI).isAlive())
				{
					if (getOwnerINLINE() == iI || GET_PLAYER((PlayerTypes)iI).hasHeadquarters(eIndex))
					{
						...
						if (getOwnerINLINE() == iI)
						{
							...
							bool bFirst = true;
							for (int iJ = 0; iJ < GC.defines.iNUM_CORPORATION_PREREQ_BONUSES; ++iJ)
							{
...
{
	FAssert(iIndex >= 0);
	FAssert(iIndex < GC.defines.iMAX_TRADE_ROUTES);
...
int CvCity::getTradeRoutes() const
{
	...
	return std::min(iTradeRoutes, GC.defines.iMAX_TRADE_ROUTES);
}
...
void CvCity::updateTradeRoutes()
{
	...
	paiBestValue = new int[GC.defines.iMAX_TRADE_ROUTES];
	...
	if (!isDisorder() && !isPlundered())
	{
		...
		FAssert(iTradeRoutes <= GC.defines.iMAX_TRADE_ROUTES);
			...
			if (GET_PLAYER(getOwnerINLINE()).canHaveTradeRoutesWith((PlayerTypes)iI))
			{
				for (pLoopCity = GET_PLAYER((PlayerTypes)iI).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER((PlayerTypes)iI).nextCity(&iLoop))
				{
					if (pLoopCity != this)
					{
						...
						{
							if (pLoopCity->plotGroup(getOwnerINLINE()) == plotGroup(getOwnerINLINE()) || GC.defines.iIGNORE_PLOT_GROUP_FOR_TRADE_ROUTES)
							{
// BUG - Fractional Trade Routes - start
...
/*************************************************************************************************/
			...
			int iProductionGold = ((iLostProduction * GC.defines.iMAXED_UNIT_GOLD_PERCENT) / 100);
			// UNOFFICIAL_PATCH End
...
// BUG - Overflow Gold Fix - start
			...
			int iProductionGold = ((iLostProduction * GC.defines.iMAXED_BUILDING_GOLD_PERCENT) / 100);
...
// BUG - Overflow Gold Fix - start
			...
			int iProductionGold = ((iLostProduction * GC.defines.iMAXED_PROJECT_GOLD_PERCENT) / 100);
// BUG - Overflow Gold Fix - end
//Multiple Production: Added by Denev 07/02/2009
			m_iGoldFromLostProduction = iProductionGold;
//Multiple Production: End Add
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.defines.iUSE_GROWTH_CALLBACK==1)
	{
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.defines.iUSE_CULTURE_CALLBACK==1)
	{
        CyCity* pyCity = new CyCity(this);
        CyArgsList argsList;
        argsList.add(gDLL->getPythonIFace()->makePythonObject(pyCity));	// pass in city class
...
bool CvCity::doCheckProduction()
{
	...
	for (iI = 0; iI < GC.getNumUnitInfos(); iI++)
	{
		if (getUnitProduction((UnitTypes)iI) > 0)
		{
			...
			{
				iProductionGold = ((getUnitProduction((UnitTypes)iI) * GC.defines.iMAXED_UNIT_GOLD_PERCENT) / 100);
	...
	for (iI = 0; iI < GC.getNumBuildingInfos(); iI++)
	{
		if (getBuildingProduction((BuildingTypes)iI) > 0)
		{
			...
			{
				iProductionGold = ((getBuildingProduction((BuildingTypes)iI) * GC.defines.iMAXED_BUILDING_GOLD_PERCENT) / 100);
	...
	for (iI = 0; iI < GC.getNumProjectInfos(); iI++)
	{
		if (getProjectProduction((ProjectTypes)iI) > 0)
		{
			...
			{
				iProductionGold = ((getProjectProduction((ProjectTypes)iI) * GC.defines.iMAXED_PROJECT_GOLD_PERCENT) / 100);
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.defines.iUSE_DOPRODUCTION_CALLBACK==1)
	{
        CyCity* pyCity = new CyCity(this);
        CyArgsList argsList;
        argsList.add(gDLL->getPythonIFace()->makePythonObject(pyCity));	// pass in city class
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.defines.iUSE_DORELIGION_CALLBACK==1)
	{
        CyCity* pyCity = new CyCity(this);
        CyArgsList argsList;
        argsList.add(gDLL->getPythonIFace()->makePythonObject(pyCity));	// pass in city class
						...
						if (GET_PLAYER((PlayerTypes)iJ).isAlive())
						{
							for (pLoopCity = GET_PLAYER((PlayerTypes)iJ).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER((PlayerTypes)iJ).nextCity(&iLoop))
							{
								if (pLoopCity->isConnectedTo(this))
								{
									...
									if (iSpread > 0)
									{
										iSpread /= std::max(1, (((GC.defines.iRELIGION_SPREAD_DISTANCE_DIVISOR * plotDistance(getX_INLINE(), getY_INLINE(), pLoopCity->getX_INLINE(), pLoopCity->getY_INLINE())) / GC.getMapINLINE().maxPlotDistance()) - 5));
...
//FfH: End Add
					...
					if (GC.getGameINLINE().getSorenRandNum(GC.defines.iRELIGION_SPREAD_RAND, "Religion Spread") < iRandThreshold)
					{
						setHasReligion(((ReligionTypes)iI), true, true, true);
						break;
					}
...
/**						                                            							**/
/*************************************************************************************************/
	if(GC.defines.iUSE_DOGREATPEOPLE_CALLBACK==1)
	{
        CyCity* pyCity = new CyCity(this);
        CyArgsList argsList;
        argsList.add(gDLL->getPythonIFace()->makePythonObject(pyCity));	// pass in city class
...
void CvCity::liberate(bool bConquest)
{
	...
	if (NO_PLAYER != ePlayer)
	{
		...
		if (NULL != pPlot)
		{
			...
			{
				for (int i = 0; i < GC.defines.iCOLONY_NUM_FREE_DEFENDERS; ++i)
				{
...
//<<<<Unofficial Bug Fix: End Add
		...
		{
			CvTraitInfo& kTrait = GC.getTraitInfo( ( (TraitTypes) GC.getCivilizationInfo( getCivilizationType() ).getCivTrait() ) );
...
//*** Each religion has equal chance of adopting.
/*
					if (GC.getGameINLINE().getSorenRandNum(100, "Religion Adoption") <= GC.defines.iRELIGION_ADOPTION_CHANCE)
					{
...
//<<<<Unofficial Bug Fix: End Add
		...
		{
			pUnit->setHasPromotion((PromotionTypes)GC.defines.iMUTATED_PROMOTION, true);
		}
	}
...
//>>>>Unofficial Bug Fix: Added by Denev 2010/02/21
	...
	argsList.add(gDLL->getPythonIFace()->makePythonObject(pyCity));	// pass in city class
	gDLL->getPythonIFace()->callFunction(PYGameModule, "applyBuildEffects", argsList.makeFunctionArgs());
...
{
    int iCrime = GC.getGameINLINE().getCrime() + m_iCrime;
    if (iCrime < 0)
    {
        return 0;
    }
...
int CvCity::getAltarLevel()
{
    ...
    if (isHasBuildingClass((BuildingClassTypes)GC.getInfoTypeForString("BUILDINGCLASS_ALTAR_OF_THE_LUONNOTAR")))
        return 1;

    if (isHasBuildingClass((BuildingClassTypes)GC.getInfoTypeForString("BUILDINGCLASS_ALTAR_OF_THE_LUONNOTAR_ANOINTED")))
        return 2;

    if (isHasBuildingClass((BuildingClassTypes)GC.getInfoTypeForString("BUILDINGCLASS_ALTAR_OF_THE_LUONNOTAR_BLESSED")))
        return 3;

	if (isHasBuildingClass((BuildingClassTypes)GC.getInfoTypeForString("BUILDINGCLASS_ALTAR_OF_THE_LUONNOTAR_CONSECRATED")))
        return 4;

	if (isHasBuildingClass((BuildingClassTypes)GC.getInfoTypeForString("BUILDINGCLASS_ALTAR_OF_THE_LUONNOTAR_DIVINE")))
        return 5;

	if (isHasBuildingClass((BuildingClassTypes)GC.getInfoTypeForString("BUILDINGCLASS_ALTAR_OF_THE_LUONNOTAR_EXALTED")))
        return 6;

	if (isHasBuildingClass((BuildingClassTypes)GC.getInfoTypeForString("BUILDINGCLASS_ALTAR_OF_THE_LUONNOTAR_FINAL")))
        return 7;
