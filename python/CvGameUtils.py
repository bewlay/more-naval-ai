...
#						return 0
	...
	def AI_chooseTech(self,argsList):
		...
		iCurrRel=pPlayer.getStateReligion()
...
# -- More Naval AI - Set Waterlog factor for tech choice
		if CyMap().getMapScriptName() == "Archipelago":
			coastProdMod = 2
		else:
			coastProdMod = 1
		if pPlayer.getNumCities() > 0:
			iWaterlogFactor = coastProdMod + float(float(pPlayer.countNumCoastalCities()) / float(pPlayer.getNumCities()))
		else:
			iWaterlogFactor = 1


# More Naval AI - Give naval techs high priority if Waterlogged
		if iWaterlogFactor >= 2:
			if eTeam.isHasTech(gc.getInfoTypeForString('TECH_EXPLORATION')) == False:
				return gc.getInfoTypeForString('TECH_EXPLORATION')
			elif eTeam.isHasTech(gc.getInfoTypeForString('TECH_FISHING')) == False:
				return gc.getInfoTypeForString('TECH_FISHING')
#			elif eTeam.isHasTech(gc.getInfoTypeForString('TECH_SAILING')) == False:
#				return gc.getInfoTypeForString('TECH_SAILING')
# Make sure we hit Exploration early
		elif eTeam.isHasTech(gc.getInfoTypeForString('TECH_EXPLORATION')) == False:
			return gc.getInfoTypeForString('TECH_EXPLORATION')	

# Set Tier and Econ status		
...
#Civ Specific 		
			...
			if iCiv == gc.getInfoTypeForString('CIVILIZATION_HIPPUS'):
				if eTeam.isHasTech(gc.getInfoTypeForString('TECH_HORSEBACK_RIDING')) == False:
...
# Tholal AI - Early Priests
			if (iCurrRel != ReligionTypes.NO_RELIGION):
				if (iFavRel == iCurrRel):
					if eTeam.isHasTech(gc.getInfoTypeForString('TECH_PHILOSOPHY')) == False:
						iTech = gc.getInfoTypeForString('TECH_PHILOSOPHY')
					elif eTeam.isHasTech(gc.getInfoTypeForString('TECH_PRIESTHOOD')) == False:
						iTech = gc.getInfoTypeForString('TECH_PRIESTHOOD')		
...
#----------------------------			
# More Naval AI - Beeline Sailing if lots of coastal cities once initial economy established
#----------------------------	
		if bEcon1 and (iWaterlogFactor > 1.5):
			if eTeam.isHasTech(gc.getInfoTypeForString('TECH_SAILING'))== False:
				if pPlayer.countNumCoastalCities() > 0:
					iTech = gc.getInfoTypeForString('TECH_SAILING')	

#----------------------------			
# More Naval AI - Beeline Optics if lots of coastal cities once economy and military established
#----------------------------	
		if (bEcon2 and bTier2) and (iWaterlogFactor > 1.5):
			if eTeam.isHasTech(gc.getInfoTypeForString('TECH_OPTICS'))== False:
				if pPlayer.countNumCoastalCities() > 0:
					iTech = gc.getInfoTypeForString('TECH_OPTICS')

#----------------------------			
# More Naval AI - Beeline Astronomy if lots of coastal cities and Tier 3 military
#----------------------------	
		if (bTier3 and bEcon2) and (iWaterlogFactor > 1.5):	
			if eTeam.isHasTech(gc.getInfoTypeForString('TECH_ASTRONOMY'))== False:
				if pPlayer.countNumCoastalCities() > 2:
...
# More Naval AI	- Push AI towards early Fishing if coastal bound
			if iWaterlogFactor >= 2:
				lValues[sTechs.index('TECH_FISHING')]+=300		
		...
		#Fresh Water				
								...
								if not pLoopPlot.isHills():
									...
									if not (pLoopPlot.getTerrainType() == gc.getInfoTypeForString('TERRAIN_DESERT')):																					
										if (iFeature != gc.getInfoTypeForString('FEATURE_FOREST') or bBuildinForrest):
											...
											lValues[sTechs.index('TECH_MINING')]+=14
			...
			#Fishing
						#	if pCity.isCoastal(5):
							if pLoopPlot.isWater():
								lValues[sTechs.index('TECH_FISHING')]+=55
			...
			#Crafting													
										...
										if eBonus == gc.getInfoTypeForString('BONUS_MARBLE') :
											lValues[sTechs.index('TECH_CRAFTING')]+=50
			#Calendar																							
