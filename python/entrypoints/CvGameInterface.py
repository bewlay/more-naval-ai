...
def applyBuildEffects(argsList):
	return gameUtils().applyBuildEffects(argsList)
##--------		Unofficial Bug Fix: End Add				--------##

def AI_isPlotSpecialImprovement(argsList):
	'Returns true if the plot is of interest to AI explorers'
	return 0 
	
def AI_MageTurn(argsList):
	return gameUtils().AI_MageTurn(argsList)

def AI_MagePromotion(argsList):
	return gameUtils().AI_MagePromotion(argsList)

def AI_Mage_UPGRADE_MANA(argsList):
	return gameUtils().AI_Mage_UPGRADE_MANA(argsList)

def AI_TowerMastery(argsList):
	return gameUtils().AI_TowerMastery(argsList)