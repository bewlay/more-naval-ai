...
# BUG - Production Grouping - end
		...
		# Keys have the form (columnIdx, cityID)
		self._tooltipCache = {} # type: Dict[Tuple[int, int], unicode]
...
# BUG - Colony Split - end
	...
	def getCellHelpText( self, iColumn, iCityID ) :
		...
		if self.isShowingTooltips() :
			return self._tooltipCache.get( (iColumn, iCityID), u"" )
		return u""
...
# BUG - Colony Split - end
	...
	def drawBasicScreen (self):
		...
		screen.addCheckBoxGFC( self.TOGGLE_TOOLTIPS_NAME, ArtFileMgr.getInterfaceArtInfo( "INTERFACE_BTN_DEMOGRAPHICS" ).getPath(),
				...
				self.nControlSize, self.nControlSize, WidgetTypes.WIDGET_CDA_TOGGLE_TOOLTIPS, -1, -1,
...
#					szEffects += u"%d%s " % (iEffect, self.commerceIcons[eCommerceType])
	...
	def canAdviseToConstruct(self, city, i):
		...
		if (i == BuildingTypes.NO_BUILDING):
			return False
		info = gc.getBuildingInfo(i)
...
#						if city.findBaseYieldRateRank(YieldTypes.YIELD_COMMERCE) < 6:
						...
						value += info.getCommerceChange(CommerceTypes.COMMERCE_CULTURE) * 10
						if value > bestData:
							bestOrder = bldg
							bestData = value
#						else:
...
#						value = info.getPowerValue() / float(info.getProductionCost())
						value = info.getFlavorValue(0) # 0 is FLAVOR_MILITARY
						if value > bestData:
							bestOrder = bldg
							bestData = value
					elif type == "Nutty":
						...
						if (info.getReligionType() != ReligionTypes.NO_RELIGION):
							value = 1
							if value > bestData:
								bestOrder = bldg
								bestData = value
						...
						value += info.getCommerceChange(CommerceTypes.COMMERCE_RESEARCH) * 10
						if value > bestData:
							bestOrder = bldg
							bestData = value
					elif type == "Spaceship":
	...
	def drawContents (self, page):
		...
		if(self.customizing):
			...
			for i, column in enumerate(columns):
				# CDA_REFACTOR 03/2021 lfgr
				if column[0] in self.COLUMNS_INDEX : # Catch bad column names, e.g. from previous versions.
					...
					if isinstance( columnDef, CDAColumns.CDAColumn ) :
						...
						for i, t in enumerate( lCitiesWithHelpers ) :
							...
							try :
								...
								elif columnDef.type == "date" :
									screen.setTableDate( page, value + 1, i, szCellValue, "", WidgetTypes.WIDGET_CDA_CELL, iData1, iData2, CvUtil.FONT_RIGHT_JUSTIFY )
								else :
									screen.setTableText( page, value + 1, i, szCellValue, "", WidgetTypes.WIDGET_CDA_CELL, iData1, iData2, CvUtil.FONT_LEFT_JUSTIFY )
							except Exception, e : # LFGR_TODO: Seems to get eaten otherwise (?)
								CvUtil.pyPrint( str( e ) )
								import traceback
								traceback.print_exc()
