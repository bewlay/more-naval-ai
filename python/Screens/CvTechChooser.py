...
## End Advanced Diplomacy
		...
		for j in range( gc.getNumCivicInfos() ):
			if (gc.getCivicInfo(j).getTechPrereq() == i):
				if (gc.getCivicInfo(j).getPrereqCivilization() == -1 or gc.getCivicInfo(j).getPrereqCivilization() == gc.getGame().getActiveCivilizationType()): ##FFH Prereq CIv
		...
		for j in range( gc.getNumProjectInfos() ):
			if (gc.getProjectInfo(j).getTechPrereq() == i):
				if (gc.getProjectInfo(j).getPrereqCivilization() == -1 or gc.getProjectInfo(j).getPrereqCivilization() == gc.getGame().getActiveCivilizationType()): ## FFH - PreReq Civs
