...
#			CyInterface().addMessage(0, True, 25, CyTranslator().getText("TXT_KEY_MESSAGE_START_PRODUCTION_ACHERON", (pCity.getName(), )), '', 0, '', ColorTypes(11), pPlot.getX(), pPlot.getY(), True, True)
			szMessage = CyTranslator().getText("TXT_KEY_MESSAGE_START_PRODUCTION_ACHERON", (pCity.getName(), ))
			szButton = gc.getUnitInfo(iUnit).getButton()
			for iPlayer in range(gc.getMAX_CIV_PLAYERS()):
				if gc.getPlayer(iPlayer).isAlive():
					CyInterface().addMessage(iPlayer, True, gc.getEVENT_MESSAGE_TIME(), szMessage, '', InterfaceMessageTypes.MESSAGE_TYPE_INFO, szButton, gc.getInfoTypeForString('COLOR_UNIT_TEXT'), pCity.getX(), pCity.getY(), True, True)
##--------		Unofficial Bug Fix: End Modify			--------##
