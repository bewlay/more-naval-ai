...
def lookupFunction(module, functionOrClass, log=True):
	...
	try:
		...
		raise ConfigError( "Error while looking up function or class '%s' in module '%s': %s",
				functionOrClass, module, e )
...
def bindFunction(obj, functionOrAttribute):
	...
	try:
		...
		raise ConfigError( "Error while looking up function or attribute '%s' in object of type '%s': %s",
				functionOrAttribute, obj.__class__, e )
