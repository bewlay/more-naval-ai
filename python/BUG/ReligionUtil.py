...
##--------	BUGFfH: Added by Denev 2009/10/12
def isPriest(info, iReligion):
	"""
	Returns True if <info> is the Priest for <iReligion>.
	"""
	bPriest = false
	if	info.getPrereqBuilding() == getBuilding(iReligion, BUILDING_TEMPLE)\
	and	info.getFreePromotions(gc.getInfoTypeForString("PROMOTION_DIVINE"))\
	and	info.getFreePromotions(gc.getInfoTypeForString("PROMOTION_CHANNELING2")):
		bPriest = true
